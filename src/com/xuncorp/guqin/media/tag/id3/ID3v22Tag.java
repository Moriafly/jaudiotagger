...
import com.xuncorp.guqin.media.FileConstants;
import com.xuncorp.guqin.media.audio.mp3.MP3File;
import com.xuncorp.guqin.media.logging.ErrorMessage;
import com.xuncorp.guqin.media.tag.*;
import com.xuncorp.guqin.media.tag.datatype.DataTypes;
import com.xuncorp.guqin.media.tag.id3.framebody.*;
import com.xuncorp.guqin.media.tag.id3.valuepair.ImageFormats;
...
public class ID3v22Tag extends AbstractID3v2Tag {
    ...
    private void readHeaderFlags(ByteBuffer byteBuffer) throws TagException {
        ...
        if (unsynchronization) {
            logger.config(ErrorMessage.ID3_TAG_UNSYNCHRONIZED.getMsg(""));
        }

        if (compression) {
            logger.config(ErrorMessage.ID3_TAG_COMPRESSED.getMsg(""));
        }

        //Not allowable/Unknown Flags
        if ((flags & FileConstants.BIT5) != 0) {
            logger.warning(ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg("", FileConstants.BIT5));
        }
        if ((flags & FileConstants.BIT4) != 0) {
            logger.warning(ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg("", FileConstants.BIT4));
        }
        if ((flags & FileConstants.BIT3) != 0) {
            logger.warning(ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg("", FileConstants.BIT3));
        }
        if ((flags & FileConstants.BIT2) != 0) {
            logger.warning(ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg("", FileConstants.BIT2));
        }
        if ((flags & FileConstants.BIT1) != 0) {
            logger.warning(ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg("", FileConstants.BIT1));
        }
    ...
    public void read(ByteBuffer byteBuffer) throws TagException {
        ...
        logger.config("Reading tag from file");
        ...
        logger.config("Loaded Frames,there are:" + frameMap.keySet().size());
    }
